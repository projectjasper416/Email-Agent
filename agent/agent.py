...
A coordination note that is not about copy quality but about system design: there is a separate legacy email cron that fires templated messages on account-age days 3, 5, 7, 30, and 45 for users who have not uploaded a resume or have not tailored. You do not need to coordinate with it, defer to it, or reason about its schedule. The system already keeps you out of its lane — users in their first week who have not tailored yet are filtered out before they ever reach you — and a hard 4-day frequency cap guarantees no user is emailed twice within four days by either system. So every user in the list you receive is genuinely yours to decide on: evaluate them on their own merits. Never skip someone with a reason like "legacy cron handles this window" — if a user reached you, the legacy cron is not the reason to pass.
